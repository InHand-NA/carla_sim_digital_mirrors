...
"""
添加了y_offset参数后，ISO8855坐标系原点在车辆中心线上
"""

START_X_OFFSET = 20 # pixels, 忽略图像底部20个像素的空间信息
...
class CameraGeometry(object):
    def __init__(self, height=1.3, yaw_deg=0, pitch_deg=-5, roll_deg=0, image_width=1024, image_height=512, field_of_view_deg=45,
                 y_offset=0):
        """
        y_offset: 相机安装位置相对于车辆中心的横向偏移量，单位为米，以司机视角向左偏移为正
        """
        self.debug = True
        # scalar constants
        ...
        self.set_pose(height, yaw_deg, pitch_deg, roll_deg, y_offset)
        self.cut_v = 0
        self.cut_x = 999.0
        self.start_x = 0
    ...
    def set_pose(self, height=1.3, yaw_deg=0, pitch_deg=-5, roll_deg=0,
                 vp_u=0, vp_v=0, y_offset=0):
        """
        Set the pose of the camera
        This function should be called when the extrinsic parameters are estimated
        y_offset: 相机安装位置相对于车辆中心的横向偏移量，单位为米，以司机视角向左为正
        """
        self.height = height
        self.y_offset = y_offset
    ...
    def uv_to_roadXYZ_roadframe_iso8855(self,u,v):
        ...
        return np.array([Z,-X + self.y_offset,-Y]) # read book section on coordinate systems to understand this

    def roadXYZ_roadframe_iso8855_to_uv(self,X,Y,Z):
        #print(f"roadXYZ_roadframe_iso8855_to_uv: {X}, {Y}, {Z}")
        roadframe_xyz = np.array([-(Y - self.y_offset), -Z, X]) # iso8855 frame to road frame
        polyline_world = np.array([roadframe_xyz])
        uv = project_polyline(polyline_world, self.trafo_road_to_cam, self.intrinsic_matrix)
        #print(f"uv: {uv}")
        u, v = uv[0][0], uv[0][1]
        # Fixme: v may be smaller than cut_v
        return u, v

    def roadXYZ_roadframe_iso8855_to_uv_polyline(self, road_x: List[float], road_y: List[float], road_z: List[float]):
        roadframe_x = -np.array(road_y - self.y_offset)
        ...
        for i in range(len(roadframe_x)):
            if roadframe_z[i] < self.start_x:
                print(f"Skip point {i} because it is nearer than start_x={round(self.start_x, 2)}m")
                continue
            if roadframe_z[i] > self.cut_x:
                print(f"Skip point {i} because it is farther than cut_x={round(self.cut_x, 2)}m")
                continue
            polyline_world.append(np.array([roadframe_x[i], roadframe_y[i], roadframe_z[i]]))
        polyline_world = np.array(polyline_world)
        # 添加空数组检查
        if len(polyline_world) == 0:
            return np.array([]).reshape(0, 2)  # 返回空的2D数组        
        uv = project_polyline(polyline_world, self.trafo_road_to_cam, self.intrinsic_matrix)
        # filter points with v < cut_v
        uv = uv[uv[:, 1] >= self.cut_v]
        return uv
    ...
    def update_cut_data(self):
        vp_u, vp_v = self.get_vp_from_pitch_yaw(self.intrinsic_matrix)
        self.cut_v = vp_v + 1
        x, y, z = self.uv_to_roadXYZ_roadframe_iso8855(self.image_width / 2.0, self.cut_v)
        self.cut_x = x
        x, y, z = self.uv_to_roadXYZ_roadframe_iso8855(self.image_width / 2.0, self.image_height - START_X_OFFSET)
        self.start_x = max(x, 0.0)
        print(f"update_cut_data: cut_v={self.cut_v}, cut_x={round(self.cut_x, 2)}m, start_x={round(self.start_x, 2)}m")
        return vp_u, vp_v
    ...
    def save_ipm_json_params(self, file_path):
        ...
        with open(file_path, 'w') as f:
            json.dump({
                ...
                "camera_y_offset": float(self.y_offset),
    ...
    def load_ipm_json_params(self, file_path):
        ...
        if "camera_y_offset" in params:
            self.y_offset = params["camera_y_offset"]
        else:
            self.y_offset = 0
        self.timestamp = params["timestamp"]
        self.set_pose(self.height, self.yaw_deg, self.pitch_deg, self.roll_deg, self.vp_u, self.vp_v, self.y_offset)
        self.update_cut_data()
        print(f"Loaded IPM params from {file_path}: cut_v={self.cut_v}, cut_x={round(self.cut_x, 2)}m, start_x={round(self.start_x, 2)}m, image_width={self.image_width}, image_height={self.image_height}, camera_height={self.height}, camera_pitch={self.pitch_deg}deg, camera_yaw={self.yaw_deg}deg, camera_roll={self.roll_deg}deg, timestamp={self.timestamp}")
